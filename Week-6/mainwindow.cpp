...
void MainWindow::on_ontgrendelSchuifMetId_clicked()
{
    ...
    try {
        if (voordeurKaartSlot->ontgrendel(id)) {
            voordeur->openen();
            ui->exceptionBox->append("Toegang verleend voor ID: " + QString::fromStdString(id));
        }
    } catch (const SlotException &e) {
        QString errorMsg = QString("EXCEPTION: Plaats: %1 | ID: %2")
                               .arg(QString::fromStdString(e.getPlaats()))
                               .arg(QString::fromStdString(e.getId()));
        ui->exceptionBox->append(errorMsg);
    }
